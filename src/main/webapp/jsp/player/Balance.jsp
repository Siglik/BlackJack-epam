...
<a href="#" class="button payment" id="toogle-payment">balance operations</a> <br>
<div id="payment-form" class="hidden">
	<form class="payment" method="get" action="/blackjack/$/player/payment/">
		
		    <label>Sum:</label> <input type="text" name="sum" size="9" pattern="[0-9]{1,6}(\.[0-9]{1,2})?" required/> <span
			class="wrapper"> &nbsp;</span> 
			<label>Card number:</label> <input
			type="text" name="number" size="18" pattern="^(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|6(?:011|5[0-9][0-9])[0-9]{12}|3[47][0-9]{13}|3(?:0[0-5]|[68][0-9])[0-9]{11}|(?:2131|1800|35\d{3})\d{11})$" required/> <br>
			<label>Expirience date:</label> 
			<input type="text" name="exp-month" size="2" pattern="[0-9]{2}" required/> 
			<input type="text" name="expr-year" size="2" pattern="[0-9]{2}" required/> <span class="wrapper">&nbsp;</span> 
			<label>CVV-code:</label> <input type="text" name="swift" size="4" pattern="[0-9]{3,4}" required/><br>
		    <label>Cardholder name:</label> <input type="text" name="cardholder" pattern="[a-zA-Z]{2,64}[ ]+[a-zA-Z]{2,64}" required/><br>
		<div class="bth-container">
			<button name="operation" class="button" value="pay">PAY</button>
